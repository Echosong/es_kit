layui.define(['layer', 'element', 'fortree'], function (exports) {
    ...
    function bind(height) {
        var height = height || 60 + 41 + 0; //头部高度 顶部切换卡标题高度 底部高度
        /**
         * iframe自适应
         */
        ...
        $('.t_slide').click(function (event) {
            if ($(this).hasClass('slide')) {
                $(this).removeClass('slide');
                $('.m-side').animate({left: '-200px'}, 200).siblings('.m_body').animate({left: '0px'}, 200);
            } else {
                $(this).addClass('slide');
                $('.m-side').animate({left: '0px'}, 200).siblings('.m_body').animate({left: '200px'}, 200);
            }
        });
    }
    ...
    function clickLI(index) {
        nav.find('dd').eq(index || 0).parent("dl").parent("li").addClass("layui-nav-itemed");
        nav.find('dd').eq(index || 0).click();
    }
